...
const pythonTemplate = (problemNumber, url, title, description, testCase) => {
  ...
  const pythonTestCase = testCase
    .replace(/expect\(answer\("(.+?)"\)\)\.toBe\("(.+?)"\)/, 'self.assertEqual(answer("$1"), "$2")')
    .replace(/expect\(answer\((.+?)\)\)\.toBe\((.+?)\)/, 'self.assertEqual(answer($1), $2)');
...
async function extractProblems(html, chapter) {
  ...
  $('h2').each((i, heading) => {
    ...
    const titleMatch = $(heading).text().match(/\d+\.\s*(.*)/);
    ...
    const url = `https://nlp100.github.io/ja/ch${chapter.toString().padStart(2, '0')}.html#${headingId}`;
...
async function getProblemsForChapter(chapter ) {
  ...
  const url = `https://nlp100.github.io/ja/ch${chapter.toString().padStart(2, '0')}.html`;
  try {
    const html = await fetchHtml(url);
    return await extractProblems(html, chapter);
  } catch (error) {
    console.error(`Error getting problems for chapter ${chapter}: ${error.message}`);
    throw error;
  }
}
...
async function generateFile(problemNumber, language, chapter) {
  try {
    ...
    console.log(`問題 ${problemNumber} のデータを取得しました: ${problem.title}`);
    ...
    if (fs.existsSync(fileName)) {
      ...
      if (!overwrite) {
        console.error(`ファイル ${fileName} は既に存在します。上書きするには FORCE_OVERWRITE=true を設定してください。`);
        return false;
      }
    }
    ...
    console.error(`ファイル ${problemNumber} の生成中にエラーが発生しました:`, error.message);
    return false;
  }
}

// Function to generate files for a range of problems
async function generateFilesForRange(startNumber, endNumber, language, chapter) {
  let success = true;
...
async function main() {
  ...
  try {
    if (number) {
      ...
      const success = await generateFilesForRange(start, end, language, chapter);
      process.exit(success ? 0 : 1);
    }
  } catch (error) {
